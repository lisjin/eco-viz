...
PREF = sys.argv[1].replace('Mindful', 'M').replace('Rest', 'R').replace('.', '').replace('+', '-')
edges = []
for filename in os.listdir(INPUT_DIR):
	...
	if os.path.isfile(fpath):
		...
		for idx, line in enumerate(f):
			...
			src = line.split()[0]
			dest = line.split()[1]
			edge['_from'] = PREF + '-nodes' + '/' + 'n' + src
			edge['_to'] = PREF + '-nodes' + '/' + 'n' + dest
			edge['id'] = 'e' + str(idx)
			edge['tstep'] = tstep
			edges.append(edge)
		f.close()
		
with open(PREF + '-edges.json', 'w') as edge_outf:
	edge_outf.write(unicode(json.dumps(edges, indent=4, separators=(',', ': '))))
edge_outf.close()

# with open(PREF + '-nodes.json', 'w') as node_outf:
